...
function edit(type) {
    ...
    var index = editor.innerHTML.indexOf(text);
    ...
    if (overlap == null) {
        ...
        if (s.anchorNode.previousSibling) {
            var offset = editor.innerHTML.indexOf(s.anchorNode.previousSibling.outerHTML) + s.anchorNode.previousSibling.outerHTML.length;
            indexText += offset;
            indexText2 += offset;
        }
        ...
        if (comment.value.length > 0) {
            if (type) {
                var newtext = editor.innerHTML.slice(0, indexText) + "<span id='" + uuid + "' class='highlight' onmouseover=\"hover('" + uuid + "')\" onmouseout=\"out('" + uuid + "')\">"
                            + editor.innerHTML.slice(indexText, indexText2) + "</span>" + editor.innerHTML.slice(indexText2);
                editor.innerHTML = newtext;
            } else {
                var newtext = editor.innerHTML.slice(0, indexText) + "<span id='" + uuid + "' class='strike' onmouseover=\"hover('" + uuid + "')\" onmouseout=\"out('" + uuid + "')\">"
                            + editor.innerHTML.slice(indexText, indexText2) + "</span>" + editor.innerHTML.slice(indexText2);
                editor.innerHTML = newtext;
            }
